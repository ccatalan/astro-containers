...
// clean redundancies in L1-L2-L3 definitions
...
// see how to limit installed dependencies ok, could do more
...
// Maybe also spline interpolation is not the best choice in that case since it procudces wiggles which can be ill-interpreted.
...
func sfit(gallist,base,&_ki,&_resi,nMC=,MC=,verb=,maxit=,maxitMC=,noskip=,meval=,mevalMC=,meval3=,kin=,epar=,MASK=,guess=,mus=,vlim=,sav=,asc=,RMASK=,zlim=,rmin=,bf=,padv=,frtol1=,N=,nor=,nde=,sigdef=,L1=,L2=,L3=,L4=,muc=,co=,pl=,mucov=,cov=,parage=,RG=,muv=,mux=,muz=,mue=,mub1=,mub2=,z2d=,bnpec=,splin=,__sigm=,AMRp=,muAMRp=,rrAMRp=,Nb=,lsp=,plavgW=){
  /* DOCUMENT
  ...
  ***************************************************************************
     ...
     by default Nb=1 -> working with flux fractions
  ...
  */
  ...
  for(j=1;j<=ns;j++){
    ...
    npec=0;
      ...
      nssigm=ssigm;
      if(!is_void(dimsof(where(sW<=2.e-2)))) nssigm(where(sW<=2.e-2))=1.e10;
      ...
      for(i=1;i<=nMC;i++){
        ...
        if(0){ write,"clipping E(B-V)";
        for(it=1;it<=maxitMC;it++){  // E(B-V) clipping
