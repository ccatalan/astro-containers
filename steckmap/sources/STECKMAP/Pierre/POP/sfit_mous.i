...
//      pmodel1=model;
    ...
    npec=0;
	  ...
	  // copying the data + the best model into an ascii file. M. M. (26-06-2008)
	  f=open(u.resfile(1)+"-SED.txt","w");
	  write,f,numberof(base.wave);
	  write,f,"Wavelengths"
	  write,f,base.wave;
	  write,f,"Observed spectrum"
	  write,f,d;
	  write,f,"Best model"
	  write,f,pmodel1;
      ...
      for(i=1;i<=nMC;i++){
        ...
        if (MC=="MC") { // REGULAR MC
          ...
          // d=d  choose the data to start the MC simulatins; dont forget to 
          // comment the line above, i.e., d=pmodel1 and hope for the best!
          snr=u.SNR;
          // using the error spectrum to re-sample the spectrum -- MM & PO
          // d=d+sigm*random_normal(numberof(d))
...
//      write,"determining E(B-V)";
      ...
      sads=sfrs=amrs=masses=gres(:nab,)*0.;
        ...
        write,f,numberof(base.ages);
        write,f,base.ages;
        //write,f,"SAD (flux fractions)",numberof(sad);
        write,f,numberof(sad);
        write,f,sad^2;
        sads(,1)=sad^2;
        if(!is_void(nMC)){
