...
func bRbasis2(ages,&FWHMout,&bab,mets=,nbins=,Rtarget=,FWHMangstrom=,FWHMpix=,wavel=,N=,basisfile=,dlambda=,zr=,br=,inte=,navg=,list=,intages=,dirsfr=,filters=,s=){
  ...
  */
  ...
  if (is_void(dlambda)) dlambda=[]; 
  if (!is_void(R)) {if (R>10000.) {write, "you dreamin', man !";error;};};
  if (!is_void(FWHMpix)) FWHM=FWHMpix;
  if (is_void(N)) N=1;
  if(is_void(navg)) navg=0;
  if(is_void(zr)) zr=1;
  if(is_void(intages)) intages=0;
  ...
  dd=dib(2); 
  if(R==Res) R=[];
  //  if(is_void(Rtarget)) write,"Using nominal model resolution";
 ...
 //************************************************
 // Compute FWHM (pix) to convolve with to obtain R in case FWHMpix was not supplied
 //************************************************
 if ((!is_void(Rtarget))){
   //&&(R<10000.)){
  FWHM1=(_x0(avg)/Res)/(_x0(dif)(avg)); // FWHM of basis in pixels
  FWHM2=(wavel(avg)/Rtarget)/(_x0(dif)(avg)); // FWHM to obtain in pixels
  FWHM=sqrt((FWHM2^2-FWHM1^2));
  write,"convolving with Gaussian of "+pr1(FWHM)+" pixels";
};

 if(!is_void(FWHMpix)) FWHM=FWHMpix;

 if(!is_void(FWHM)) write,"convolving with Gaussian of "+pr1(FWHM)+" pixels";
 ...
 for(i=1;i<=nbins;i++){
   ...
   //if (navg!=1) asp(,i,)=integ(bloc(:dd,ibab(i):ibab(i+1),imet),ta( ibab(i):ibab(i+1)),ta( ibab(i):ibab(i+1))(0),2);
   //   if ((!is_void(R))|(!is_void(FWHM))) { // commented and replaced by line below 14/05/2010 PO
   if(!is_void(FWHM)){
     for(j=1;j<=numberof(imet);j++){
 ...
 if (!is_void(dlambda)) {
   dlambda=dlambda==0?(wavel(avg))/(2.*(is_void(Rtarget)?Res:Rtarget)):dlambda;
   ndd=int((wavel(0)-wavel(1))/dlambda);
   nwave=wavel(1)+dlambda*indgen(ndd);
   nasp=array(0.,ndd,nbins,numberof(imet));
   for(i=1;i<=nbins;i++){
   ...
   if (!is_void(br)){
     if(is_void(dlambda)) dlambda=(_x0(dif))(avg);
   np=int((wavel(0)+wavel(1))/(2.*(is_void(Rtarget)?Res:Rtarget))*(1./dlambda))+1;
   asp=asp(np:-np,,);
   };
   ndd=dimsof(asp)(2);
 ...
 base.met=is_void(zr)?_m(imet):Zrescale(_m(imet));
 base.R=is_void(Rtarget)?Res:Rtarget;
 base.age_unit="yr";
...
func bRbasis3(ages,&FWHMout,&bab,mets=,nbins=,R=,wavel=,N=,basisfile=,dlambda=,zr=,br=,inte=,navg=,list=,intages=,dirsfr=,filters=,s=,FWHMpix=){
  /* DOCUMENT
  ...
  */
  ...
  base=bRbasis2(ages,&FWHMout,&bab,mets=mets,nbins=nbins,Rtarget=R,wavel=wavel,N=0,basisfile=basisfile,dlambda=dlambda,zr=zr,br=br,inte=inte,navg=navg,list=list,intages=intages,dirsfr=dirsfr,filters=filters,s=s,FWHMpix=FWHMpix);
  ...
  base=bRbasis2(ages,&FWHMout,&bab,mets=mets,nbins=nbins,Rtarget=R,wavel=wavel,N=N,basisfile=basisfile,dlambda=dlambda,zr=zr,br=br,inte=inte,navg=navg,list=list,intages=intages,dirsfr=dirsfr,filters=filters,s=s,FWHMpix=FWHMpix);
