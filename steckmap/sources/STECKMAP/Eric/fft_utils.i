 ...
 *
 * History:
 *	$Id: fft_utils.i,v 1.7 2003/08/23 09:55:57 eric Exp eric $
 *	$Log: fft_utils.i,v $
   ...
   SEE ALSO: fft_indgen, fft_symmetric_index. */
{
  ...
  /* Build square radius array one dimension at a time, starting with the
     last dimension. */
  local r2;
  if ((n = numberof(dimlist)) == 1) return 0.0; /* scalar array */
  if (is_void(nyquist)) {
    for (k=n ; k>1 ; --k) {
      u = fft_indgen(dimlist(k));
      r2 = (k<n ? r2(-,..) + u*u : u*u);
    }
  } else {
    nyquist *= 2.0;
...
func fft_profile(iu,&px)
/* DOCUMENT fft_profile(iu,px);
     returns the profile of the power spectrum of the image u
     takes the fourrier transform of u as its argument
   SEE ALSO:
 */
{
  dd= dimsof(iu)(1);
  ns= dimsof(iu)(0);
  x1=(fft_indgen(ns)-1);
if(dd==2)  r1= abs(x1, x1(-,));
if(dd==3)  r1= abs(x1, x1(-,),x1(-,-,));
  py= histo2(r1, px, weight=abs2(iu), average=1, interp=1,binsize=r1(dif)(min));
  return py;
}
