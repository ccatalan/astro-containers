 ...
 *
 * History:
 *	$Id: lmfit.i,v 1.3 1998/09/08 15:31:06 eric Exp $
 *	$Log: lmfit.i,v $
 ...
 */
{
	...
	if (anyof(gamma <= 0.0)) {
        ...
        // change PO 13/12/09 this should be better cause gamma is not always same dimensionality
        for(ig=1;ig<=numberof(gamma);ig++){
          gamma(ig)=(gamma(ig)==0?0:1.0/gamma(ig));
        };
	//gamma= 1.0 / gamma; makes SIGFPE error when gamma=0
        // end change PO 13/11/09
	beta *= gamma;
	alpha *= gamma(,-) * gamma(-,);
